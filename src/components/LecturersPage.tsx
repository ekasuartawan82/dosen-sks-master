...
import { useLecturers, useDeleteLecturer } from "@/hooks/useLecturers";
...
const LecturersPage = () => {
    ...
    const { data: lecturers, isLoading } = useLecturers(selectedProgram, false, activeAcademicYear?.name);
    ...
    const handleDelete = async (lecturerId: string, lecturerName: string) => {
        try {
            ...
            const lecturerAssignments = assignments?.filter(a => a.lecturer_id === lecturerId) || [];
            ...
            await deleteLecturer.mutateAsync(lecturerId);
